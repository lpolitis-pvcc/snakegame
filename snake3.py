...
# Snake
snake = [(9, 9)] # puts snake in middle
...
# Functions
def draw_block(pos, color, radius=1): # defines radius in block drawing so we can alter snake/food roundedness
...
def draw_background():
    for x in range(GRID_SIZE):
        for y in range(GRID_SIZE):
            color = DARK_GREEN if (x+y) % 2 == 0 else (0, 60, 0) # % determines whether a number is even/odd
...
while True:
    ...
    if not game_over:
        ...
        if direction != (0, 0):  # Snake is moving
            ...
            new_head = (head_x + direction[0], head_y + direction[1]) # calculates new head position based on direction
    ...
    for segment in snake:
        draw_block(segment, GREEN, radius=3) # makes the snake the slightest bit rounded

    draw_block(food, RED, radius=20) # makes the food round like an apple
    draw_score(screen)
    ...
    pygame.display.flip()
    clock.tick(11)  # Control the speed
